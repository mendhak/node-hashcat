...
//
HTMLProcessor.prototype.replaceWithRevved = function replaceWithRevved(lines, revvedFiles) {
    ...
    regexps.forEach(function (rxl) {
      ...
      content = content.replace(rxl[0], function (match, src) {
        ...
        var srcfile = filterIn(src);
          ...
          var lookup = {};
          for (var i = 0, len = revvedFiles.length; i < len; i++) {
              lookup[revvedFiles[i].file] = revvedFiles[i].revved;
          }

        var file = lookup[srcfile];
        if(!file)
        {
            file = src;
        }
...
//
HTMLProcessor.prototype.process = function process(revvedFiles) {
  return this.replaceWithRevved(this.replaceBlocks(), revvedFiles);
};
